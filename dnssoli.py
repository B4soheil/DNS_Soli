...
def start_cooking(interface, nametodns, nameservers, tcp=False, ipv6=False, port="53", logfile=None):
    try:
        ...
        if logfile: 
            ...
            log.write("[%s] DNSSoli is active.\n" % (time.strftime("%d/%b/%Y:%H:%M:%S %z")) )
        else:
            log = None

        if tcp:
            print "[*] DNSSoli is running in TCP mode"
        ...
        if log:
            log.write("[%s] DNSSoli is shutting down.\n" % (time.strftime("%d/%b/%Y:%H:%M:%S %z")) )
            log.close()

        server.shutdown()
        print "[*] DNSSoliis shutting down."
        sys.exit()
...
if __name__ == "__main__":
    ...
    parser = OptionParser(usage = "dnssoli.py [options]:\n" + header, description="DNSSoli is a highly configurable DNS Proxy for Penetration Testers and Malware Analysts. It is capable of fine configuration of which DNS replies to modify or to simply proxy with real responses. In order to take advantage of the tool you must either manually configure or poison DNS server entry to point to DNSSoli. The tool requires root privileges to run on privileged ports." )
    ...
    print "[*] DNSSoli started on interface: %s " % options.interface
